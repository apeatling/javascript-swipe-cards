...
var SwipeCards = (function () {
	...
	var _setCardClasses = function() {
		var beforeCard = cardEls.before.classList;
		var activeCard = cardEls.active.classList;
		var afterCard = cardEls.after.classList;
		
		if ( 'up' == direction ) {
			beforeCard.remove( 'before' );
			beforeCard.add( 'after' );

			activeCard.remove( 'active' );
			activeCard.remove( 'drag-complete' );
			activeCard.add( 'before' );

			afterCard.remove( 'after' );
			afterCard.add( 'active' );
		} else {
			beforeCard.remove( 'before' );
			beforeCard.add( 'active' );

			activeCard.remove( 'active' );
			activeCard.remove( 'drag-complete' );
			activeCard.add( 'after' );

			afterCard.remove( 'after' );
			afterCard.add( 'before' );					
		}
	};
